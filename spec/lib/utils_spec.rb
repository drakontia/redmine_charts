...
module RedmineCharts
  describe Utils do
    ...
    it 'should return routings' do
      ...
      expect(result).to eq([%w(burndown charts_burndown), %w(burndown2 charts_burndown2), %w(ratio charts_ratio), %w(timeline charts_timeline), %w(deviation charts_deviation), %w(issue charts_issue)])
    end
