...
describe ConditionsUtils do
  ...
  it "should return project_its_subprojects_ids" do
    expect(@utils.project_and_its_children_ids(15041)).to eq([15041,15042])
  end
  ...
  it "should return_proper_conditions" do
    ...
    expect(options[:project_ids]).to eq([["Project1", 15041], ["Project2", 15042]])
    expect(options[:user_ids]).to eq([["Dave Lopper", 15043], ["John Smith", 15042], ["Redmine Admin", 15041]])
    expect(options[:assigned_to_ids]).to eq([["Dave Lopper", 15043], ["John Smith", 15042], ["Redmine Admin", 15041]])
    expect(options[:author_ids]).to eq([["Dave Lopper", 15043], ["John Smith", 15042], ["Redmine Admin", 15041]])
    expect(options[:activity_ids]).to eq([["Design", 8], ["Development", 9]])
    expect(options[:category_ids]).to eq([["Project1 - Category1", 15041], ["Project1 - Category2", 15042], ["Project2 - Category3", 15043]])
    expect(options[:fixed_version_ids]).to eq([["Project1 - 1.0", 15041], ["Project1 - 2.0", 15042]])
  ...
  it "should set project_ids if not provided in params" do
    ...
    expect(conditions[:project_ids]).to eq([15041, 15042])
  end
