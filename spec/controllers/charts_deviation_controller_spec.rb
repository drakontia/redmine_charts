...
Rspec.describe ChartsDeviationController, type: :controller do
  ...
  skip 'not DRY yet' do
    ...
    expect(body['elements'][0]['values'][1][0]['tip'].gsub('\\u003C', '<').gsub('\\u003E', '>').gsub("\000", '')).to eq("#{l(:charts_deviation_hint_logged, logged_hours: 5.6)}#{l(:charts_deviation_hint_issue, estimated_hours: 10.0, work_done: 100)}#{l(:charts_deviation_hint_label, issue_id: 15_041, issue_name: 'Issue1')}")
    ...
    expect(body['elements'][0]['values'][2][0]['tip'].gsub('\\u003C', '<').gsub('\\u003E', '>').gsub("\000", '')).to eq("#{l(:charts_deviation_hint_logged, logged_hours: tmp)}#{l(:charts_deviation_hint_issue, estimated_hours: 8.0, work_done: 60)}#{l(:charts_deviation_hint_label, issue_id: 15_043, issue_name: 'Issue3')}")

    expect(body['elements'][0]['values'][2][1]['val']).to be_within(1).of(63.0)
    expect(body['elements'][0]['values'][2][1]['tip'].gsub('\\u003C', '<').gsub('\\u003E', '>').gsub("\000", '')).to eq("#{l(:charts_deviation_hint_remaining_over_estimation, remaining_hours: 5.1, hours_over_estimation: 4.7, over_estimation: 58)}#{l(:charts_deviation_hint_issue, estimated_hours: 8.0, work_done: 60)}#{l(:charts_deviation_hint_label, issue_id: 15_043, issue_name: 'Issue3')}")

    expect(body['elements'][0]['values'][3][0]['val']).to be_within(1).of(74.0)
    expect(body['elements'][0]['values'][3][0]['tip'].gsub('\\u003C', '<').gsub('\\u003E', '>').gsub("\000", '')).to eq("#{l(:charts_deviation_hint_logged, logged_hours: 8.9)}#{l(:charts_deviation_hint_issue, estimated_hours: 12.0, work_done: 60)}#{l(:charts_deviation_hint_label, issue_id: 15_045, issue_name: 'Issue5')}")

    expect(body['elements'][0]['values'][3][1]['val']).to be_within(1).of(49.0)
    expect(body['elements'][0]['values'][3][1]['tip'].gsub('\\u003C', '<').gsub('\\u003E', '>').gsub("\000", '')).to eq("#{l(:charts_deviation_hint_remaining_over_estimation, remaining_hours: 6.0, hours_over_estimation: 2.9, over_estimation: 24)}#{l(:charts_deviation_hint_issue, estimated_hours: 12.0, work_done: 60)}#{l(:charts_deviation_hint_label, issue_id: 15_045, issue_name: 'Issue5')}")
    ...
    expect(body['x_axis']['labels']['labels'][1]).to eq(l(:charts_deviation_label, issue_id: 15_041))
    expect(body['x_axis']['labels']['labels'][2]).to eq(l(:charts_deviation_label, issue_id: 15_043))
    expect(body['x_axis']['labels']['labels'][3]).to eq(l(:charts_deviation_label, issue_id: 15_045))
  ...
  skip 'not DRY yet' do
      ...
      expect(body['elements'][0]['values'][1][0]['tip'].gsub('\\u003C', '<').gsub('\\u003E', '>').gsub("\000", '')).to eq("#{l(:charts_deviation_hint_logged, logged_hours: 13.2)}#{l(:charts_deviation_hint_issue, estimated_hours: 12.0, work_done: 0)}#{l(:charts_deviation_hint_label, issue_id: 15_047, issue_name: 'Issue Parent')}")
      expect(body['elements'][0]['values'][1][1]['val']).to be_within(1).of(100.0)
      expect(body['elements'][0]['values'][1][1]['tip'].gsub('\\u003C', '<').gsub('\\u003E', '>').gsub("\000", '')).to eq("#{l(:charts_deviation_hint_remaining_over_estimation, remaining_hours: 12.0, hours_over_estimation: 13.2, over_estimation: 110)}#{l(:charts_deviation_hint_issue, estimated_hours: 12.0, work_done: 0)}#{l(:charts_deviation_hint_label, issue_id: 15_047, issue_name: 'Issue Parent')}")
      ...
      expect(body['elements'][0]['values'][2][0]['tip'].gsub('\\u003C', '<').gsub('\\u003E', '>').gsub("\000", '')).to eq("#{l(:charts_deviation_hint_logged, logged_hours: 3.3)}#{l(:charts_deviation_hint_issue, estimated_hours: 5.0, work_done: 0)}#{l(:charts_deviation_hint_label, issue_id: 15_048, issue_name: 'Issue Child 1')}")
      expect(body['elements'][0]['values'][2][1]['val']).to be_within(1).of(100.0)
      expect(body['elements'][0]['values'][2][1]['tip'].gsub('\\u003C', '<').gsub('\\u003E', '>').gsub("\000", '')).to eq("#{l(:charts_deviation_hint_remaining_over_estimation, remaining_hours: 5.0, hours_over_estimation: tmp, over_estimation: 66)}#{l(:charts_deviation_hint_issue, estimated_hours: 5.0, work_done: 0)}#{l(:charts_deviation_hint_label, issue_id: 15_048, issue_name: 'Issue Child 1')}")

      expect(body['elements'][0]['values'][3][0]['val']).to be_within(1).of(94.3)
      expect(body['elements'][0]['values'][3][0]['tip'].gsub('\\u003C', '<').gsub('\\u003E', '>').gsub("\000", '')).to eq("#{l(:charts_deviation_hint_logged, logged_hours: 6.6)}#{l(:charts_deviation_hint_issue, estimated_hours: 7.0, work_done: 0)}#{l(:charts_deviation_hint_label, issue_id: 15_049, issue_name: 'Issue Child 2')}")
      expect(body['elements'][0]['values'][3][1]['val']).to be_within(1).of(100.0)
      expect(body['elements'][0]['values'][3][1]['tip'].gsub('\\u003C', '<').gsub('\\u003E', '>').gsub("\000", '')).to eq("#{l(:charts_deviation_hint_remaining_over_estimation, remaining_hours: 7.0, hours_over_estimation: 6.6, over_estimation: 94)}#{l(:charts_deviation_hint_issue, estimated_hours: 7.0, work_done: 0)}#{l(:charts_deviation_hint_label, issue_id: 15_049, issue_name: 'Issue Child 2')}")

      tmp = ActiveRecord::Base.connection.adapter_name =~ /postgresql|sqlite/i ? 3.4 : 3.3

      expect(body['elements'][0]['values'][4][0]['val']).to be_within(1).of(47.1)
      expect(body['elements'][0]['values'][4][0]['tip'].gsub('\\u003C', '<').gsub('\\u003E', '>').gsub("\000", '')).to eq("#{l(:charts_deviation_hint_logged, logged_hours: 3.3)}#{l(:charts_deviation_hint_issue, estimated_hours: 7.0, work_done: 0)}#{l(:charts_deviation_hint_label, issue_id: 15_050, issue_name: 'Issue Child 2.1')}")
