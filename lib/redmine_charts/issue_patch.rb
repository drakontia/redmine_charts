  ...
  module IssuePatch
    ...
    module InstanceMethods
      def chart_remove_issue
        ChartDoneRatio.delete_all(issue_id: id)
        ChartIssueStatus.delete_all(issue_id: id)
        ChartTimeEntry.delete_all(issue_id: id)
      end

      def chart_update_issue_done_ratio
        RedmineCharts::IssuePatch.add_chart_done_ratio(project_id, id, status_id, done_ratio, Time.now)
      end

      def chart_update_issue_status
        RedmineCharts::IssuePatch.add_chart_issue_status(project_id, id, status_id, Time.now)
      end
    end
  end
end
