...
class IssuePatchTest < ActiveSupport::TestCase
  def test_issue_status
    ...
    issue = Issue.new(project_id: 15_041, tracker_id: 1, author_id: 1, status_id: 1, priority: IssuePriority.all.first, subject: 'test_create', description: 'IssueTest#test_create')
    assert issue.save
    issue.reload
    ...
    assert_equal 2_010_070, issue_status[0].day
    assert_equal 2_010_010, issue_status[0].week
    assert_equal 2_010_003, issue_status[0].month
    assert_equal issue.id, issue_status[0].issue_id
    assert_equal 15_041, issue_status[0].project_id
  ...
  def test_done_ratio
    ...
    assert_equal 15_041, done_ratio[0].project_id
    assert_equal 10, done_ratio[0].done_ratio

    assert_equal 2_010_070, done_ratio[1].day
    assert_equal 2_010_010, done_ratio[1].week
    assert_equal 2_010_003, done_ratio[1].month
    assert_equal issue.id, done_ratio[1].issue_id
    assert_equal 15_041, done_ratio[1].project_id
  ...
  def test_done_ratio_creation_with_done_ratio
    issue = Issue.new(project_id: 15_041, tracker_id: 1, author_id: 1, status_id: 1, priority: IssuePriority.all.first, subject: 'test_create', description: 'IssueTest#test_create', done_ratio: 20)
    assert issue.save
    issue.reload
